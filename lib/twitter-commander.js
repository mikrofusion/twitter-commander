...
viewManager.addCommand('clear', '', 'prompt', function() { /*return gob.vanish();*/ });
...
viewManager.addView('timeline', function(data) {
    //gob.vanish();
...
viewManager.addView('prompt', function() {
    return TwitterCommander.setPrompt(ioInterface);
  }, function() {  /*ioInterface, key*/
...
TwitterCommander.run = function(creds) {
  ...
  Bacon.fromEventTarget(ioInterface, 'close').onValue(function() {
    console.log('\nbye.');
